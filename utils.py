...
def kill_model(model_number=None):
    ...
    if model_number is None:
        ...
        for i in range(len(model_info["Model"])):
            if model_info["Model"][i] == model_number:
                idx = i
...
def destroy_data(idx):
    info_data = None
    with open(init.TRAINING_DATA_INFO, "r") as f:
        info_data = json.load(f)
    
    number = info_data["Data Number"][idx]
    filename = init.TRAINING_DATA_FOLDER + "/training_data_" + str(number) + ".pkl"
    ...
    info_data["Iterations"].pop(idx)
    info_data["Models"].pop(idx)
...
def post_game_training(hivemind_manager, sequential_data, episodes, window):
    ...
    if len(sequential_data) < 5:
        ...
        batches_per_epoch = init.BATCHES_PER_EPOCH // 4
        epochs = init.TRAINING_EPOCHS // 4
    elif len(sequential_data) < 25:
        batch_size = init.TRAINING_BATCH_SIZE // 2**2
        batches_per_epoch = init.BATCHES_PER_EPOCH // 2
        epochs = init.TRAINING_EPOCHS // 2
    elif len(sequential_data) < 50:
        batch_size = init.TRAINING_BATCH_SIZE // 2
        batches_per_epoch = (3 * init.BATCHES_PER_EPOCH) // 4
        epochs = (3 * init.TRAINING_EPOCHS) // 4
    else:
        batch_size = init.TRAINING_BATCH_SIZE
        batches_per_epoch = init.BATCHES_PER_EPOCH
        epochs = init.TRAINING_EPOCHS
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch + 1}/{epochs} finished in {time} seconds")
        text = "Epoch: " + str(epoch) + "/" + str(epochs) + " | Loss: " + str(round(avg_loss.item(), 5)) + " | Total Time: " + str(total_time)
