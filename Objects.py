...
class AI(Object):
    ...
    def check_for_glue_collision(self, glues):
        for glue in glues:
            if self.hitbox.colliderect(glue.hitbox):
                self.in_glue = True
                self.glue_collisions += 1
                return
        self.in_glue = False

    def moving_into_wall(self):
        ...
        if dx > 0 and x + init.PLAYER_DIM >= self.window_x - 10:
            return True
        elif dx < 0 and x <= 10:
            return True
        elif x + init.PLAYER_DIM >= self.window_x:
            return True
        elif x <= 0:
            return True
        
        if dy > 0 and y + init.PLAYER_DIM >= self.window_y - 10:
            return True
        elif dy < 0 and y <= 10:
            return True
        elif y + init.PLAYER_DIM >= self.window_y:
            return True
        elif y <= 0:
            return True
    ...
    def moving_towards_player(self, player, axis='x'):
        ...
        if velocity < 0 and player_location < ai_location:
            return True
        if (ai_location - d < player_location + d and ai_location + d > player_location - d):
            return True
