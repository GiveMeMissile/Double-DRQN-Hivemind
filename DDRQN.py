...
from utils import get_lowest, copy_raw_tensor_data
...
class AIHivemindManager: # HIVEMIND TIME!!!
    ...
    reward_count = 0
    q_value_count = 0
    q_value_train_count = 0
    data_count = -1
    player_reward_count = 0
    glue_reward_count = 0
    player_save_count = 0
    glue_save_count = 0
    ...
    def set(self, glues, player, dim, color):
        # Sets the game variables at the beginning of every episode.
    ...
    def create_ais(self, num_ais, dim, color):
        # Creates the AI objects to interact with the enviorment.
    ...
    def add_frame_to_memory(self, frame):
        # Adds the new frame to the Hivemind's memory
    ...
    def update_memory(self):
        # Creates tensor to be added to the Hivemind's memory
    ...
    def move_ais(self, current_time, sync):
        ...
        self.previous_memory = self.memory.detach().clone()
    ...
    def calculate_reward(self):
        # Calculates the rewards for the current frame.
        ...
        for i, ai in enumerate(self.ai_list):
            ...
            max_distance = math.sqrt((init.WINDOW_X)**2 + (init.WINDOW_Y) **2)  # PYTHAGOREAN THEOREM AGAIN?!?!?!?!? AMAZING!!!!!!!!
            normalized_distance = distance/max_distance
            reward += math.sqrt(1-(normalized_distance))*init.DISTANCE_REWARD_MULTIPLIER
    ...
    def save_data(self):
        ...
        self.data_manager.append([
            copy_raw_tensor_data(self.previous_memory[self.num_saved_frames - 1]).to(init.device),
            actions, 
            copy_raw_tensor_data(self.memory[self.num_saved_frames - 1]).to(init.device), 
            reward
        ])
        self.data_count += 1
        self.save_memories()
        for ai in self.ai_list:
            if ai.in_glue:
                self.glue_save_count += 1
            if ai.touching_player:
                self.player_save_count += 1

    def sequence_training(self, sequences, timesteps_till_backdrop=5):
        # Trains the policy model on the inputted sequence.
        ...
        for s in range(sequence_length):
            ...
            self.q_value_train_avg += q_values.mean().item()
            self.q_value_train_count += 1
            ...
            # print(f"\nPrev State: {states.shape} | Actions: {actions} | New State: {new_states.shape} | Rewards: {rewards} | Target: {targets} | Avg Q: {avg_q} | avg_loss: {total_loss}\n") 
            ...
            if (timesteps_till_backdrop <= steps_since_backdrop) or (s == sequence_length - 1):
                ...
                accumulated_loss = 0
                steps_since_backdrop = 0
        avg_loss = total_loss / (sequence_length//timesteps_till_backdrop) if (sequence_length//timesteps_till_backdrop) > 0 else total_loss
    ...
    def match_model(self):
        # Tries to match the current model with one which already exists.
    ...
    def save_memories(self):
        # Debug function, used to save data for later testing.

        self.memories.append((
            self.memory.detach().clone(),
            self.data_count, 
            self.ai_list[0].in_glue, 
            self.ai_list[0].touching_player, 
            self.ai_list[0].moving_into_wall(),
            self.ai_list[0].moving_towards_player(self.player, "x"),
            self.ai_list[0].moving_towards_player(self.player, "y"),
            self.ai_list[0].hitbox.x,
            self.ai_list[0].hitbox.y
        ))
    
    def save_memory(self, file):
        # Function used for debugging, saves information used for testing in a pickle file.
